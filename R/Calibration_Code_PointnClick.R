...
#----------------------------------------
#httr::set_config(httr::use_proxy("192.168.41.8", port = 80))
#library
#install.packages("mgcv")

# load calibration library (RUN FROM THIS LINE DOWN AFTER THE FIRST TIME OF USING SCRIPT)
library(loggercal)
library(mgcv)
...
internalCalDir <- choose.dir(default = internalCalDirs[1], caption = "CHOOSE INTERNAL CALIBRATION DIRECTORY")
  ...
  cat("Internal Calibration data read from:",
    "\n\t", internalCalDir, sep = ""); flush.console()
...
externalCalDir <- choose.dir(default = externalCalDirs[1], caption = "CHOOSE EXTERNAL CALIBRATION DIRECTORY")
 ...
 cat("\n\tExternal Calibration data read from:",
    "\n\t", externalCalDir, sep = ""); flush.console()
...
plot1<-tryCatch({
          ...
          plot(cal - control ~ control, data = ukas$data[[1]], pch = 16, cex = 0.5,
          ...
          plot(cal - control ~ control, data = ukas$data[[2]], pch = 16, cex = 0.5,
          ylab = "calibration logger bias", xlab = "temperature")
          lines(temp, predict(externalCalMod[[2]], list(control = temp)) - temp)
          
          }
          }, error=function(e){cat("NO EXTERNAL CALIBRATION LOGGER FOUND",conditionMessage(e), "\n")})
...
#--------------------------------------------------------------------
  ...
  write.csv(file = paste0(internalCalDir, "/", "full_calibration_coefs_", paste0(basename(internalCalDir)), ".csv"),
            tab, row.names = FALSE)
